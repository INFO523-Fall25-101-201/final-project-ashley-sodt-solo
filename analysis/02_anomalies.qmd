...
def analyze_horse_anomalies(
    ...
    z_mean = X_z.abs().mean(axis=1)   # <-- NEW: average |z| across features
...
# Example: average |z|-score vs. session across horses
plt.figure()
for horse, df_h in anomaly_summary.groupby("horse"):
    plt.scatter(df_h["session_id"], df_h["z_mean_abs"], label=horse)
